...
def prepare_alfresco():
    if os.path.isdir('alfresco'):
        ...
        fe.read()
        content = fe.get_content()
        new_content = replace_content(content, '[path_to_repository_directory]', PATH_TO_REPOSITORY_DIRECTORY)
        new_content = replace_content(new_content, '[tomcat]', TOMCAT_VERSION)
